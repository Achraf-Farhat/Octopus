...
export default function Playbooks() {
  const [activeTab, setActiveTab] = useState('playbooks') // playbooks, integrations
  ...
  const [simNodeStatus, setSimNodeStatus] = useState({}) // node_id: 'pending'|'running'|'completed'|'failed'|'waiting_approval'
  ...
  const [wizardStep, setWizardStep] = useState(1) // 1: Select type, 2: Config, 3: Test, 4: Success
  ...
  // Load playbooks and integrations
  ...
  return (
    <AppLayout>
      ...
      {isBuilderOpen ? (
        ...
        // Standard SOAR Dashboard: Tabbed layout between Playbooks Table and Integration Hub
        <>
          <header className="panel page-header">
            <div className="flex items-center justify-between">
              <div>
                <h1>Security Automation & Orchestration (SOAR)</h1>
                <p className="muted">Visual workflow playbooks and external security integrations.</p>
              </div>
              <div className="flex border border-slate-850 bg-slate-900/60 rounded-xl p-1 gap-1">
                <button
                  onClick={() => setActiveTab('playbooks')}
                  className={`px-4 py-2 rounded-lg text-xs font-semibold transition cursor-pointer border-none ${
                    activeTab === 'playbooks' 
                    ...
                    {wizardSelectedType === 'virustotal' && (
                      <div>
                        ...
                        <input
                          ...
                          placeholder="vt_mock_api_key_xxxxxxxxxxxxx"
                          className="w-full mt-1.5 px-3 py-2 rounded-lg bg-slate-950 border border-slate-850 text-slate-200 outline-none focus:border-blue-500 transition font-mono text-xs"
                        />
                      </div>
                    )}
